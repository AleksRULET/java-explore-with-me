...
public interface ViewStatsRepository extends JpaRepository<Hit, Long> {
    ...
    List<ViewStats> findUniqueUriViewStats(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("uris") List<String> uris);
    ...
    @Query("SELECT new ru.practicum.ewm.viewstats.model.ViewStats(v.app, v.uri, COUNT(v.ip)) " +
            ...
            "GROUP BY v.app, v.uri " +
            "ORDER BY COUNT(v.ip) DESC")
    List<ViewStats> findUrisViewStats(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("uris") List<String> uris);
